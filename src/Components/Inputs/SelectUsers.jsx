...
const SelectUsers = ({ selectUsers, setSelectedUsers }) => {
    ...
    const selectedUserAvaters = allUsers.filter((user) => selectUsers.includes(user._id)).map((user) => user.profileImageUrl)
        ...
        <div className="space-y-4 mt-2">
            ...
            {selectedUserAvaters.length > 0 && (
                <div className="cursor-pointer" onClick={() => setIsModalOpen(true)}>
                    <AvatarGroup avatars={selectedUserAvaters} maxVisible={3} />
                </div>
            )}
