...
const ViewTaskDetails = () => {
  ...
  const getTaskDetailsByID = async () => {
    try {
      const res = await axiosInstance.get(API_PATHS.TASKS.GET_TASKS_BY_ID(id));
      if (res.data) {
  ...
  const updateTodoChecklist = async (index) => {
    ...
    if (todoChecklist && todoChecklist[index]) {
      ...
      try {
        const res = await axiosInstance.put(API_PATHS.TASKS.UPDATE_TODO_CHECKLIST(taskId), {
          todoChecklist
        });
        if (res.status === 200) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 mt-4">
            <div className="form-card col-span-3">
              ...
              <div className="grid grid-cols-12 gap-4 mt-4">
                ...
                <div className="col-span-6 md:col-span-4">
                  <label htmlFor="" className='text-xs font-medium text-slate-500'>Assigned To</label>
              ...
              {task?.attachments?.length > 0 && (
                <div className="mt-2">
                  ...
                  {task?.attachments?.map((link, index) => (
                    <Attachments key={`link_${index}`} link={link} index={index} onClick={() => handleLinkClick(link)} />
                  ))}
                </div>
              )}
            </div>
